...
int main(void)
{
    ...
    for (int i = 0; i < PREFECTURES_COUNT; i++)
    {
        ...
        // 　　　No02:Kanagawa  ######################
        //       ※例の#の数は正確ではありません
        for (int j = 0, loop = prefectures[i].population / 250000; j < loop; j++)
        {
            printf("#");
        }
    ...
    do
    {
        ...
        for (int i = 0; i < PREFECTURES_COUNT - 1; i++)
        {
            ...
            if (prefectures[i].population < prefectures[i + 1].population)
            {
                ...
                //prefectures[i]の値を一時的に変数tmpに格納する
                prefecture tmp = prefectures[i];
                //prefecture[i]の値をprefectures[i + 1]の値で上書きする
                prefectures[i] = prefectures[i + 1];
                //prefectures[i + 1]の値をtmpの値(prefectures[i])で上書きする
                prefectures[i + 1] = tmp;
                // 交換が行われた回数をカウント
                exchange++;
            }
