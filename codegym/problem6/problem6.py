...
def main():
    ...
    while exchange > 0:
        exchange = 0
        for i in range(len(target_post_codes) - 1):
            if target_post_codes[i] > target_post_codes[i + 1]:
                tmp = target_post_codes[i]
                target_post_codes[i] = target_post_codes[i + 1]
                target_post_codes[i + 1] = tmp
                exchange += 1
